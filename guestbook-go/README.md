...
This example shows how to build a simple multi-tier web application using Kubernetes and Docker. The application consists of a web front end, Redis leader for storage, and replicated set of Redis replicas, all for which we will create Kubernetes replication controllers, pods, and services.
 ...
 * [Step One: Create the Redis leader pod](#step-one)
 * [Step Two: Create the Redis leader service](#step-two)
...
### Step One: Create the Redis leader pod<a id="step-one"></a>

Use the `examples/guestbook-go/redis-leader-controller.json` file to create a [replication controller](https://kubernetes.io/docs/concepts/workloads/controllers/replicationcontroller/) and Redis leader [pod](https://kubernetes.io/docs/concepts/workloads/pods/pod-overview/). The pod runs a Redis key-value server in a container. Using a replication controller is the preferred way to launch long-running pods, even for 1 replica, so that the pod benefits from the self-healing mechanism in Kubernetes (keeps the pods alive).

1. Use the [redis-leader-controller.json](redis-leader-controller.json) file to create the Redis leader replication controller in your Kubernetes cluster by running the `kubectl create -f` *`filename`* command:

    ```console
    $ kubectl create -f examples/guestbook-go/redis-leader-controller.json
...
2. To verify that the redis-leader controller is up, list the replication controllers you created in the cluster with the `kubectl get rc` command(if you don't specify a `--namespace`, the `default` namespace will be used. The same below):
    ...
    Result: The replication controller then creates the single Redis leader pod.
...
4. To verify what containers are running in the redis-leader pod, you can SSH to that machine with `gcloud compute ssh --zone` *`zone_name`* *`host_name`* and then run `docker ps`:
...
### Step Two: Create the Redis leader service <a id="step-two"></a>
...
Services find the pods to load balance based on pod labels. The pod that you created in Step One has the label `app=redis` and `role=leader`. The selector field of the service determines which pods will receive the traffic sent to the service.

1. Use the [redis-leader-service.json](redis-leader-service.json) file to create the service in your Kubernetes cluster by running the `kubectl create -f` *`filename`* command:

    ```console
    $ kubectl create -f examples/guestbook-go/redis-leader-service.json
...
2. To verify that the redis-leader service is up, list the services you created in the cluster with the `kubectl get services` command:
    ...
    Result: All new pods will see the `redis-leader` service running on the host (`$REDIS_MASTER_SERVICE_HOST` environment variable) at port 6379, or running on `redis-leader:6379`. After the service is created, the service proxy on each node is configured to set up a proxy on the specified port (in our example, that's port 6379).


### Step Three: Create the Redis replica pods <a id="step-three"></a>

The Redis leader we created earlier is a single pod (REPLICAS = 1), while the Redis read replicas we are creating here are 'replicated' pods. In Kubernetes, a replication controller is responsible for managing the multiple instances of a replicated pod.
...
2. To verify that the redis-replica controller is running, run the `kubectl get rc` command:
    ...
    Result: The replication controller creates and configures the Redis replica pods through the redis-leader service (name:port pair, in our example that's `redis-leader:6379`).
...
3. To verify that the Redis leader and replicas pods are running, run the `kubectl get pods` command:
    ...
    Result: You see the single Redis leader and two Redis replica pods.

### Step Four: Create the Redis replica service <a id="step-four"></a>

Just like the leader, we want to have a service to proxy connections to the read replicas. In this case, in addition to discovery, the Redis replica service provides transparent load balancing to clients.
...
This is a simple Go `net/http` ([negroni](https://github.com/codegangsta/negroni) based) server that is configured to talk to either the replica or leader services depending on whether the request is a read or a write. The pods we are creating expose a simple JSON interface and serves a jQuery-Ajax based UI. Like the Redis replica pods, these pods are also managed by a replication controller.
...
3. To verify that the guestbook pods are running (it might take up to thirty seconds to create the pods), list the pods you created in cluster with the `kubectl get pods` command:
    ...
    Result: You see a single Redis leader, two Redis replicas, and three guestbook pods.
