...
The Redis master pod we created earlier is a single pod (REPLICAS = 1), while the Redis slave pods we are creating here are 'replicated' pods. In Kubernetes, a replication controller is responsible for managing the multiple instances of a replicated pod.
